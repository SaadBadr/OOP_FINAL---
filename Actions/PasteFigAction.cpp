...
void PasteFigAction::Execute() 
{
	...
	for(int i = 0 ; i < pManager->getCount() ; i++ )
		...
		if((pManager->getFigList())[i]->isCutted()){

		(DeleteAction(pManager,true,(pManager->getFigList())[i])).Execute();
	...
	if(pManager->IsCutted){ DeleteAction(pManager,true,ClipboardFigure);
